var Parser = function(bodyDOM) {
	this.DOM = bodyDOM;
	this.tagger = new Tagger(bodyDOM);
	...
	//	Components.utils.reportError('Parser.getAddress()'); // DEBUG
		...
		var ex2 = /^\s*(\S[^\r\n]+\S)\s?(\S[^\r\n]*\S)?\s+(\S[^\r\n]+\S),\s+([A-Z]{2})\s+([0-9]{5}).*$/m;
		var results = this.DOM.textContent.match(ex);
		if (results) {
			var addresses = new Array();
		//	Components.utils.reportError(results.length + ' addresses found'); // DEBUG
